...
class RHQueueTests(unittest.TestCase):
  ...
  def test_begin_time(self):
    output_file = f"{inspect.currentframe().f_code.co_name}.stdout"
    script = subprocess.run(
        self.args("test_venv.py", b="1", o=output_file, v=self.v, e=self.e))
    self.assertEqual(script.returncode, 0)
  ...
  def test_shebang_env(self):
    output_file = f"{inspect.currentframe().f_code.co_name}.stdout"
    script = subprocess.run(
        self.args("test_shebang_env.py", o=output_file, v=self.v, e=self.e))
    self.assertEqual(script.returncode, 0)
    time.sleep(2)
  ...
  #     with self.subTest(server=f"titan{i}"):
  #       self._sub_server_test(server=f"titan{i}")

  # def _sub_server_test(self, server):
  #   output_file = f"test_multiple_{server}.stdout"
  #   script = subprocess.run(
  #       self.args("test_server_hostname.py", o=output_file, s=server))
  #   self.assertEqual(script.returncode, 0)
  #   self.assertFileContentsSame(output_file, server)
  ...
  def test_email_env(self):
    os.environ["RHQ_EMAIL"] = "peter.nicolas.castenschiold.mcdaniel@regionh.dk"
